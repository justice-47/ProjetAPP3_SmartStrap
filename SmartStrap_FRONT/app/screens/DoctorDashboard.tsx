...
// --- MOCK DATA ---
const MOCK_PATIENTS = [
  { patient_id: 101, prenom: "Lucas", nom: "Bernard", age: 45, genre: "Homme", val: 1 },
  { patient_id: 102, prenom: "Emma", nom: "Durand", age: 32, genre: "Femme", val: 2 },
  { patient_id: 103, prenom: "Léa", nom: "Petit", age: 28, genre: "Femme", val: 3 },
  { patient_id: 104, prenom: "Thomas", nom: "Moreau", age: 56, genre: "Homme", val: 4 },
];
...
const MOCK_STATS = {
  ...
  alertCount: 3,
  recentAlerts: [
    { id: 901, title: "SpO2 Basse", message: "Patient Lucas Bernard à 88%", created_at: new Date().toISOString(), prenom: "Lucas", nom: "Bernard" },
    { id: 902, title: "Arythmie", message: "Fréquence instable détectée", created_at: new Date(Date.now() - 3600000).toISOString(), prenom: "Emma", nom: "Durand" },
  ]
};
...
const KpiCard = ({ title, value, subtitle, icon, isAlert }: any) => (
  <View style={[styles.kpiCard, isAlert && styles.kpiCardAlert]}>
    <View style={styles.kpiHeader}>
      ...
      <View style={[styles.iconContainer, isAlert && { backgroundColor: "#fee2e2" }]}>
    ...
    <Text style={[styles.kpiSubtitle, isAlert && { color: "#ef4444" }]}>{subtitle}</Text>
  </View>
);
  ...
  >
    <View style={styles.alertHeader}>
      <View style={{ flexDirection: "row", alignItems: "center", flex: 1 }}>
        ...
        <View style={[styles.tag, isCritical ? {backgroundColor: "#fee2e2"} : {backgroundColor: "#fef3c7"}]}>
          <Text style={[styles.tagText, isCritical ? {color: "#991b1b"} : {color: "#92400e"}]}>{tag}</Text>
        </View>
      </View>
    ...
    >
      <View style={styles.patientHeader}>
        <View
          style={[
            styles.initials,
            isCritical ? { backgroundColor: "#fee2e2" } : { backgroundColor: "#dbeafe" },
          ]}
        >
          <Text
            style={[
      ...
      <View style={[styles.statusBadge, isCritical ? {backgroundColor: "#fecaca"} : {backgroundColor: "#d1fae5"}]}>
         <Text style={[styles.statusText, isCritical ? {color: "#991b1b"} : {color: "#065f46"}]}>
           {status}
         </Text>
      </View>
    </TouchableOpacity>
...
export default function DoctorDashboard() {
  ...
  const [stats, setStats] = useState<any>({ patientCount: 0, alertCount: 0, recentAlerts: [] });
  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      try {
        ...
        if (userId) {
          try {
            const response = await fetch(`${API_URL}/api/doctor/patients/${userId}`);
            ...
            const statsResponse = await fetch(`${API_URL}/api/doctor/dashboard-stats/${userId}`);
    ...
    <SafeAreaView style={styles.container} edges={['top', 'left', 'right']}>
      ...
      >
          ...
          <View style={styles.headerRight}>
            ...
            <View style={styles.profileContainer}>
              <View style={styles.avatar}>
                <Text style={styles.avatarText}>{doctorName ? doctorName.charAt(0).toUpperCase() : "M"}</Text>
              </View>
              <View>
                <Text style={styles.userName}>{doctorName || "Dr. Leblanc"}</Text>
        ...
        <View style={styles.sectionHeader}>
          <Text style={styles.greeting}>Bonjour, Dr. {doctorName || "Leblanc"}</Text>
        ...
        <View style={styles.sectionContainer}>
          ...
          {stats.recentAlerts.length === 0 ? (
            <View style={styles.emptyState}>
              <Text style={styles.emptyStateText}>Aucune alerte à signaler ✅</Text>
            </View>
          ) : (
        ...
        <View style={styles.sectionContainer}>
          ...
          {loading ? (
             <Text style={{padding: 20, color: '#6b7280'}}>Chargement des données...</Text>
          ) : (
...
const styles = StyleSheet.create({
  ...
  profileContainer: { flexDirection: "row", alignItems: "center", marginRight: 10 },
  avatar: {
    width: 36,
    height: 36,
    borderRadius: 18,
  ...
  userRole: { fontSize: 10, color: "#64748b", textTransform: 'uppercase', fontWeight: '600' },
  logoutButton: { padding: 5 },
  logoutEmoji: { fontSize: 18 },

  sectionHeader: { padding: 20, paddingBottom: 10 },
  greeting: { fontSize: 24, fontWeight: "700", color: "#1e293b", letterSpacing: -0.5 },
  date: { fontSize: 14, color: "#64748b", marginTop: 2, textTransform: 'capitalize' },
  ...
  alertHeader: { flexDirection: "row", justifyContent: "space-between", marginBottom: 8, alignItems: "center" },
  alertName: { fontWeight: "700", fontSize: 14, color: "#1e293b", marginRight: 8 },
  alertTime: { fontSize: 11, color: "#94a3b8", fontWeight: "500" },
  tag: { paddingHorizontal: 8, paddingVertical: 2, borderRadius: 6 },
  tagText: { fontSize: 10, fontWeight: "700" },
  alertMessage: { fontSize: 13, color: "#475569", lineHeight: 18, marginBottom: 12 },
  ...
  patientHeader: { flexDirection: "row", marginBottom: 16, alignItems: 'center' },
  initials: {
    width: 44,
    height: 44,
    borderRadius: 22,
