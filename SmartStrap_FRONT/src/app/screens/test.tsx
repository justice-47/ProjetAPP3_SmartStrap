...
export default function NotificationScreen() {
  const renderNotification = (item: any) => (
    <View
      key={item.id}
      style={[styles.notifItem, item.highlight && styles.notifHighlight]}
    >
      <View
        style={[
          styles.notifIconContainer,
          item.highlight && styles.iconHighlight,
        ]}
      >
        {item.type === "change" ? (
          <MaterialCommunityIcons
            name={item.icon}
            size={24}
            color={item.highlight ? "#00386A" : "white"}
          />
        ) : (
          <Ionicons
            name={item.icon}
      ...
      <View style={styles.notifContent}>
        <View style={styles.notifHeader}>
          <Text
            style={[styles.notifTitle, item.highlight && styles.textHighlight]}
          >
            {item.title}
          </Text>
          <Text
            style={[styles.notifTime, item.highlight && styles.textHighlight]}
          >
            {item.time}
          </Text>
        </View>
        {item.description ? (
          <Text
            style={[styles.notifDesc, item.highlight && styles.textHighlight]}
          >
            {item.description}
          </Text>
        ) : null}
